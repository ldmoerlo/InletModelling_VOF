...
#             TrialBoolean=True
        ...
        mg_checked=0.0
        mg_bubbleWall=0.0
        coordCenter=np.array([C_coord[1]-(U*C_time)*normalInlet[0],C_coord[2]-(U*C_time)*normalInlet[1],C_coord[3]-(U*C_time)*normalInlet[2]])
        for i in np.arange(len(coordList)):
            for j in np.arange(int(tunit/timeStepSize)):
                coordPoint=np.array([coordList[i,1]-(U*timeVal[t*int(tunit/timeStepSize)+j])*normalInlet[0],coordList[i,2]-(U*timeVal[t*int(tunit/timeStepSize)+j])*normalInlet[1],coordList[i,3]-(U*timeVal[t*int(tunit/timeStepSize)+j])*normalInlet[2]])
                if np.linalg.norm(coordPoint-coordCenter)<rg:
                    if VOFwVal[i,timeInterval*int(tunit/timeStepSize)+j,0] == 1.0: #Every cells not yet occupied by bubble
                        ...
                        mg_bubbleWall=mg_bubbleWall+coordList[i,4]*U*timeStepSize*rhog  # In this case, a cell was already filled with air, but I will add the mass of air to mg_bubbleWall to be able to check later whether a wall was intersected.                      
                    else:
                        return False,0.0          
...
for fi in np.arange(len(files)):
    ...
    for i in np.arange(len(coordList[:,0])):
        for j in np.arange(len(timeVal)):
            coordPoint=np.array([coordList[i,1]-(U*timeVal[j])*normalInlet[0],coordList[i,2]-(U*timeVal[j])*normalInlet[1],coordList[i,3]-(U*timeVal[j])*normalInlet[2]]) 
